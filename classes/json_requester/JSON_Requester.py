...
class JSON_Requester():
    ...
    def get_url_with_api_key(self):
        ...
        with open('.\\files\\api_key\\api_key.txt') as f:
            api_key = f.read()
        return uri + '?key=' + api_key

    def save_and_return_response(self, r):
    ...
    def send_request(self, url, payload):
        ...
        if r.status_code == 200:
            if len(r.text) >= 300:
                data = self.save_and_return_response(r)
            else:
